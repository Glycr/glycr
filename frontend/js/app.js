...
class GlycrApp {
  ...
  async loadAllData() {
    ...
    try {
      ...
      const promises = [this.fetchApi('/events?limit=1000&upcoming=false')];
  ...
  calculateOrganizerStats() {
    ...
    if (this.allPayouts) {
      this.allPayouts.forEach(payout => {
        if (payout.status === 'pending')   stats.pendingPayouts   += payout.amount;
        else if (payout.status === 'completed') stats.completedPayouts += payout.amount;
      });
    }

    if (this.orgRefunds) {
      this.orgRefunds.forEach(refund => {
        if (refund.status === 'pending') stats.pendingRefunds += refund.amount || 0;
      });
    }
  ...
  async refreshCurrentView() {
    ...
    switch (activeSection) {
      case 'dashboard':     await this.renderDashboardFromData(); break;
      case 'myevents-page': await this.renderMyEventsFromData();  break;
      case 'report-page':   await this.loadReportPage();          break;
      case 'payout-page':   await this.loadPayoutPage();          break;
  ...
  async renderDashboardFromData() {
    ...
    const live = this.orgEvents.filter(e => e.isPublished && !e.isCancelled && new Date(e.date) > now).length;
  ...
  async _loadPurchaseHistory() {
    ...
    container.innerHTML = tickets.map(t => {
      const eventId   = t.eventId || (t.event && (t.event._id || t.event.id));
      const ev        = this.allEvents.find(e => e._id === eventId || e.id === eventId);
      const eventName = ev ? ev.title : (t.eventTitle || t.event?.title || 'Event');
      ...
      return `<div class="purchase-item">
        <div class="purchase-item-body">
          <div class="purchase-item-title">${eventName}</div>
          <div class="purchase-item-meta">${(t.ticketType || 'TICKET').toUpperCase()} · ${t.price === 0 ? 'Free' : sym + (t.price || 0)} · ${new Date(t.purchasedAt).toLocaleDateString()}</div>
        </div>
        <div class="purchase-item-actions">
          ${canTransfer ? `<button class="btn btn-ghost btn-sm" style="color:var(--sky); border-color:rgba(125,211,252,0.3);" onclick="app.openTransferModal('${t.id || t._id}','${eventName}','${t.ticketType || 'ticket'}')"><i class="fas fa-paper-plane"></i> Transfer</button>` : ''}
          ${canRefund   ? `<button class="btn btn-ghost btn-sm" style="color:var(--coral); border-color:rgba(255,107,107,0.3);" onclick="app.openRefundModal('${t.id || t._id}','${eventName}','${t.ticketType || 'ticket'}',${t.price || 0})"><i class="fas fa-undo"></i> Refund</button>` : ''}
          ${canReview   ? `<button class="btn btn-ghost btn-sm" style="color:var(--gold); border-color:rgba(251,191,36,0.3);" onclick="app.openReviewModal('${eventId}','${eventName}')"><i class="fas fa-star"></i> Review</button>` : ''}
        </div>
      </div>`;
    }).join('');
  }
  ...
  async submitReview() {
    ...
    try { await this.fetchApi('/reviews', { method: 'POST', body: JSON.stringify({ eventId, rating: this.currentRating, text }) }); } catch {}
  ...
  async _generateTicketPDF(ticket, event) {
    ...
    doc.setFontSize(20); doc.setTextColor(45, 212, 191);
    doc.text('Glycr', 12, 18);
    doc.setFontSize(7); doc.setTextColor(200, 220, 230);
    doc.setFont('helvetica', 'normal');
    doc.text('EVENT TICKET', 12, 24);
    ...
    row('Type',  ticketType);
    if (event.organizerPhone) row('Phone', event.organizerPhone);
    if (event.organizerEmail) row('Email', event.organizerEmail);
      ...
      container.innerHTML = refunds.map(r => {
        ...
        return `<div class="org-refund-card">
          <div class="org-refund-header">
            <div>
              <div class="org-refund-title">${eventTitle} — ${(r.ticketType ? r.ticketType.toUpperCase() : '') || ''}</div>
  ...
  async loadReportPage() {
    ...
    myEvents.forEach(e => {
      const tt = this.parseTicketTypes(e.ticketTypes);
      Object.entries(tt).forEach(([type, t]) => { typeTotals[type] = (typeTotals[type] || 0) + (t.sold || 0); });
    });
  ...
  async togglePublish(id) {
    try { await this.fetchApi(`/events/${id}/publish`, { method: 'PATCH' }); await this.loadAllData(); }
    catch (err) { this.showToast(err.message); }
  }
  ...
  async deleteEvent(id) {
    ...
    try { await this.fetchApi(`/events/${id}`, { method: 'DELETE' }); await this.loadAllData(); }
    catch (err) { this.showToast(err.message); }
  }
